...
import {
    changeDayShowtimeAction, getAllShowtimesByCineplexAction,
...
function Showtime() {
    ...
    if (cineplexs.data.length > 0) {
        cineplexs.data.forEach((cineplex) => {
            const data = {
                value: cineplex.id, label: cineplex.name,
            };
            options.push(data);
        });
    }
    ...
    return (<main className="flex-shrink-0">
            <Container className="w-75">
                <Row>
                    <h3 className="text-center">POKAZY</h3>
                </Row>
                <Row style={{paddingLeft: 150, paddingRight: 142}}>
                    <Form.Group className="ms-1">
                        <Form.Label className="form-group required control-label">Wybierz kina</Form.Label>
                        <Select
                            ...
                            theme={(theme) => ({
                                ...theme, colors: {
                                    ...theme.colors, primary25: '#d4d4d4', primary: '#e71a0f',
                                },
                            })}
                        />
                    </Form.Group>
                                                ...
                                                return (<React.Fragment key={s}>
                                                        <div className={s === 0 ? '' : 'mt-3'}>
                                                            <h6 className="fw-bold">
                                                                {showtime.type_name === '2D' ? showtime.type_name + ' Napisy' : showtime.type_name}
                                                            </h6>
                                                        </div>
                                                        <div>
